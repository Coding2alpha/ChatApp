...
const UserItem = ({ user, handler, handleIsLoading, style }) => {
  const { avatar, name, _id } = user;
  console.log(user);
  return (
    <div
      className={`flex justify-center items-center space-x-2 text-lg mt-4 p-1 rounded-lg ${
        style && "border-2"
      }`}
    >
      ...
      <p className={`w-full flex items-center  ${style && "justify-center"}`}>
        {name}
      </p>
      {user.isAdded ? (
        <button
